...
$(".xq").click(function(){
	var items = ["时间","活动名称","收入来源","收入金额","支出缘由","支出金额","用款凭证","款项有关人员"];
	var str="";
    ...
    $(items).each(function(i){
		if(col==6&&j<2){
			str += "<div>"+items[j]+"："+$(that).parent().parent().prev().children().eq(j).text()+"</div>";
			j++;
			return true;
		}
		str += "<div>"+items[i]+"："+$(that).parent().parent().children().eq(i-j).text()+"</div>";
    });
});
    ...
    $('#myModal').modal('show');
		...
		$(".menu-search .search-btn").click(function(){
			
			
			var search_item = "http://www.baidu.com/s?wd=";
			search_item += $(this).parent().find("#s").val();
			
			window.open(search_item);
		});
